...
#include "daisy/crop/root/root_uptake.h"
...
// The 'uptake' base model.

double 
RootUptake::ABA_concentration () const
{ return ABAConc; }

double 
RootUptake::soil_T () const
{ return soil_temperature; }

double 
RootUptake::crown_potential () const
{ return h_x; }

double 
RootUptake::depth () const
{ return Depth; }

double 
RootUptake::water_stress () const
{ return water_stress_; }

double 
RootUptake::water_stress_days () const
{ return water_stress_days_; }

double 
RootUptake::production_stress () const
{ return production_stress_; }

void
RootUptake::set_production_stress (const double value)
{ production_stress_ = value; }

double 
RootUptake::potential_water_uptake (const double h_x,
                                    const Geometry& geo,
                                    const Soil& soil,
...
double
RootUptake::water_uptake (double Ept_,
                          const Geometry& geo,
                          const Soil& soil,
                          const SoilWater& soil_water,
                          ...
                          Treelog& msg)
{
  ...
  if (Ept_ < 0)
    {
      Treelog::Open nest (msg, "RootUptake water uptake");
  ...
  if (Ept < 0.010)
    ...
    water_stress_ = 1.0 - (total + EvapInterception) / (Ept + EvapInterception);
...
double
RootUptake::nitrogen_uptake (const Geometry& geo, const Soil& soil,
                             const SoilWater& soil_water,
                             Chemistry& chemistry,
...
void
RootUptake::output (Log& log) const
{
  ...
  output_value (water_stress_, "water_stress", log);
  output_value (water_stress_days_, "water_stress_days", log);
  output_value (production_stress_, "production_stress", log);
...
bool
RootUptake::check (const Geometry& geo, Treelog& msg) const
{
...
void
RootUptake::initialize_uptake (const Geometry& geo, const Soil& soil,
...
RootUptake::RootUptake (const BlockModel& al)
  : RootSystem (al),
    metalib (al.metalib ()),
    ...
    water_stress_ (0.0),
    water_stress_days_ (al.number ("water_stress_days")),
    production_stress_ (-1.0),
    Ept (0.0),
    H2OUpt (0.0),
    NH4Upt (0.0),
    NO3Upt (0.0)
...
static struct RootUptakeSyntax : DeclareBase
{
  static bool
  ...
  {
    frame.declare_object ("ABAprod", ABAProd::component, "\
...
fraction of the radition of that day that was received that hour.");
    ...
    frame.declare ("H2OUpt", "mm/h", Check::non_negative (), Attribute::LogOnly,
		   "H2O uptake.");
    frame.declare ("NH4Upt", "g N/m^2/h",
		   Check::non_negative (), Attribute::LogOnly,
		   "NH4-N uptake.");
    frame.declare ("NO3Upt", "g N/m^2/h",
		   Check::non_negative (), Attribute::LogOnly,
		   "NO3-N uptake.");
  } 

  RootUptakeSyntax () 
    : DeclareBase (RootSystem::component, "uptake", "\
Roots that takes up water and nitrogen.")
  { }
} RootUptake_syntax;

// root_uptake.C ends here.

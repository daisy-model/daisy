...
{ 
  ...
  tmp << "h\th\tTheta\tK\tCw2\tM\n"
      << "cm\tpF\t\tcm/h\tcm^-1\tcm^2/h\n";
  auto print_line = [&](const double h)
  {
    const double Theta = hydraulic->Theta (h);
    tmp << h;
    if (h < 0.0)
      tmp << "\t" << h2pF (h);
    else
    ...
    const double h_Theta = hydraulic->h (Theta);
    daisy_approximate (h_Theta, h);
  };
  const double h_Sat = 0;
  print_line (h_Sat);
  for (double pF: { 0.0, 1.0, 2.0, 3.0, 4.2, 5.0, 6.0, 7.0})
    print_line (pF2h (pF));
  msg.debug (tmp.str ());
}

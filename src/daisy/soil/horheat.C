                     ...
                     const double quarts, Treelog& msg)
{
  ...
  const double Theta_res = hydraulic.Theta_min ();
  daisy_assert (Theta_res >= 0.0);
  daisy_assert (hydraulic.Theta_sat > 0.0);
  daisy_assert (Theta_res < hydraulic.Theta_sat);
  const int from = double2int (floor (intervals * Theta_res));
